...
const MyBlogs = () => {
   ...
   const getAllBlogs = async () => {
      await axiosInstance.get('/api/user/getallblogs', {
         headers: {
   ...
   const updateBlog = async (blogId) => {
      ...
      try {
         const response = await axiosInstance.patch(`/api/user/updateblog/${blogId}`, formData, {
            headers: {
               'Authorization': `Bearer ${localStorage.getItem("token")}`
            },
         });
   ...
   const deleteBlog = async (blogId) => {
      ...
      if (assure) {
         try {
            const response = await axiosInstance.delete(`/api/user/deleteblog/${blogId}`, {
            ...
            <Row xs={1} md={2} className="g-4">
               {
                  AllBlogs && AllBlogs.length > 0 ?
                     AllBlogs.map((blog) => (
                        <Col key={blog._id}>
                           <Card>
                              <Card.Img variant="top" src={`http://localhost:8000${blog.photo.path}`} />
                              <Card.Body>
